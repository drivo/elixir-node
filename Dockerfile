...
RUN MIX_ENV=prod VERSION=0.1.0 MIX_ENV=prod mix release --env=prod

# set entrypoint
EXPOSE 4000
EXPOSE 3015

# docker run --rm --entrypoint cat elixir-node  /elixir_node/_build/prod/rel/elixir_node/releases/0.1.0/elixir_node.tar.gz > release.tar.gz
